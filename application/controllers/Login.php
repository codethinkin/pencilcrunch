...
class Login extends CI_Controller {
    ...
    public function index() {
        ...
        if ($this->session->userdata('institute_login') == 1)
            redirect(base_url() . 'institute/dashboard', 'refresh');
    ...
    function validate_login($email = '', $password = '') {
        ...
        // Checking login credential for institute
        $query = $this->db->get_where('institute', $credential);
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $this->session->set_userdata('institute_login', '1');
            $this->session->set_userdata('institute_id', $row->id);
            $this->session->set_userdata('login_user_id', $row->id);
            $this->session->set_userdata('name', $row->name_institute);
            $this->session->set_userdata('login_type', 'institute');
            return 'success';
        }
    ...
    function ajax_forgot_password()
    {
        ...
        // Checking credential for institute
        $query = $this->db->get_where('institute' , array('email' => $email));
        if ($query->num_rows() > 0)
        {
            $reset_account_type     =   'institute';
            $this->db->where('email' , $email);
            $this->db->update('institute' , array('password' => $new_password));
            $resp['status']         = 'true';
        }
